...
function SaveZoneConfig(zone, selectedItem)
    ...
    if not valuesList then return end
...
function CheckIsFightingZone()
    ...
    local fightingKeywords = {"Dungeon", "Raid", "Defense", "SorcerersDefense", "PirateTower", "ShadowGate"}
...
FM_Category = FarmTab:Category({
    ...
    Options = {
        ...
        {Title="Energy Upgrade", Icon=GetIcon(137070555439430)},
...
function LogicGamemodes()
    ...
    while Config.AutoDungeon do
        ...
        local isFightingZone = (string.find(currentMap, ":") and (string.find(currentMap, "Dungeon") or string.find(currentMap, "Raid") or string.find(currentMap, "Defense") or string.find(currentMap, "SorcerersDefense") or currentMap == "PirateTower" or currentMap == "ShadowGate") or (currentMap == "PirateTower" or currentMap == "ShadowGate" or currentMap == "ChristmasRaid" or currentMap == "SorcerersDefense"))
        ...
        if currentMap ~= "Unknown" then
            ...
            --     end;
                ...
                local CheckList = {
                    {ID="ChristmasRaid",    DB=nil,       Cfg=Config.TargetChristmas,   Key="ChristmasModeKey",         CanOpen=true,  SelName="Christmas Raid"},
...
GameModesDrop = FM_Add("GameModes", FarmTab:Dropdown({
    ...
    ImagePadding = 6,
    Callback = function(val)
        local dungeonT, raidT, defenseT, christmasT, pirateT, shadowT, sorcerersT = {}, {}, {}, {}, {}, {}, {}
...
local function IsSafeReturnZoneName(zoneName)
    ...
    if zoneName == "ChristmasRaid" then return false end
    return true
end
...
function Optimizer.Update_EnergyUpgrade_Logic(pData)
    ...
    local cost = (currentLvl * currentLvl + 1) * 50
