            ...
            if (os.time() - LastAutoSettingCheck) > 2 then
                ...
                if Data and Data.Settings and Reply and Reply.To then
                    ...
                    if Data.Settings.AutoClick == false then
                        pcall(function()
                            Reply.To("Settings","AutoClick",true);
                        end);
                    end;
...
pirateTowerList = LoadSingleGamemodeData(RS.Scripts.Configs.Gamemodes.PirateTower, "Pirate Tower", "PirateTower", "MAX_FLOOR", "Max Floors")
shadowGateList = LoadSingleGamemodeData(RS.Scripts.Configs.Gamemodes.ShadowGate, "Shadow Gate", "ShadowGate", "MAX_WAVE", "Max Waves")
capitalRaidList = LoadSingleGamemodeData(RS.Scripts.Configs.Gamemodes.CapitalRaid, "Capital Raid", "CapitalRaid", "MAX_WAVE", "Max Waves")
magicRaidList = LoadSingleGamemodeData(RS.Scripts.Configs.Gamemodes.MagicRaid, "Magic Raid", "MagicRaid", "MAX_WAVE", "Max Waves")
ignitionRaidList = LoadSingleGamemodeData(RS.Scripts.Configs.Gamemodes.IgnitionRaid, "Ignition Raid", "IgnitionRaid", "MAX_WAVE", "Max Waves")
slimeRaidList = LoadSingleGamemodeData(RS.Scripts.Configs.Gamemodes.SlimeRaid, "Slime Raid", "SlimeRaid", "MAX_WAVE", "Max Waves")
sorcerersDefenseList = LoadSingleGamemodeData(RS.Scripts.Configs.Gamemodes.SorcerersDefense, "Sorcerers Defense", "SorcerersDefense", "MAX_WAVE", "Max Waves")

-- [3. DUNGEON DATA]
function LoadDungeonData()
    local success, module = pcall(function()
        return require(RS.Scripts.Configs.Gamemodes.Dungeon);
    end);
...
function LoadRaidData()
    local success, module = pcall(function()
        return require(RS.Scripts.Configs.Gamemodes.Raid);
    end);
    if success and module and module.PHASES then
        raidList, raidDB = {}, {};
        for id, phase in ipairs(module.PHASES) do
...
function LoadDefenseData()
    local success, module = pcall(function()
        return require(RS.Scripts.Configs.Gamemodes.Defense);
    end);
    if success and module and module.PHASES then
...
GameModesDrop = FM_Add("GameModes", FarmTab:Dropdown({
    ...
    Callback = function(val)
        ...
        local singleTargetMap = {
            ...
            SlimeRaid = slimeRaidT, -- Tambahkan ini,
        }
    ...
    while not Window.Destroyed do
        if Config.AutoLeave then
            ...
            if HudGamemode and HudGamemode.Visible then
                ...
                -- Letakkan di bawah pengecekan MagicRaid
                if not modeActive then
                    local ignitionUI = HudGamemode:FindFirstChild("SlimeRaid")
                    if ignitionUI and ignitionUI.Visible then
                        local txt = ignitionUI.wave.amount.Text 
                        currentVal = tonumber(string.match(txt, "%d+")) or 0
                        targetLimit = Config.AutoLeave_SlimeRaid
                        modeActive = true
                    end
                end
...
task.spawn(function()
    ...
    -- 1. [TRACKING] TAMBAH TARGET: Saat Aura Muncul
    ...
    -- 2. [TRACKING] HAPUS TARGET: Saat Musuh Mati
    ...
    -- 3. [TRACKING] RESET: Saat Pindah Zone
    shared.Reply.Connect("Enemies Changed", function()
        IncandescentTargets = {}
    end)

    -- 4. [ACTION] LOOP PEMBURU (Smart Auto Farm Logic)
    task.spawn(function()
        while not Window.Destroyed do
            if Config.AutoMeteor then
                ...
                if hrp and humanoid and humanoid.Health > 0 then
                    ...
                    -- A. Cari Target Valid dari List Incandescent
                    local targetUID = nil
                    for uid, _ in pairs(IncandescentTargets) do
                        if enemiesData[uid] and enemiesData[uid].Alive then
                            targetUID = uid
                            break -- Fokus satu musuh dulu
                        else
                            IncandescentTargets[uid] = nil -- Bersihkan data sampah
                        end
                    end

                    local targetEnemy = targetUID and enemiesData[targetUID]

                    -- B. JIKA TARGET DITEMUKAN -> KEJAR DENGAN LOGIKA AUTO FARM
                    if targetEnemy and targetEnemy.Root then
                        pcall(function()
                            local enemyRoot = targetEnemy.Root
                            
                            -- Kalkulasi Posisi (Smart Positioning)
                            local enemyPos = enemyRoot.Position
                            local enemyLook = enemyRoot.CFrame.LookVector
                            
                            -- Hitung Tinggi Musuh (Biar sejajar)
                            local enemyScale = 1
                            ...
                            -- Posisi: 5 Stud di DEPAN musuh (Hitbox Ideal)
                            local dirVector = Vector3.new(enemyLook.X, 0, enemyLook.Z).Unit 
                            local attackPos = enemyPos + (dirVector * 5) 
                            ...
                            -- C. Teleport & Serang
                            hrp.CFrame = CFrame.lookAt(finalPos, lookAtPos)
                            hrp.AssemblyLinearVelocity = Vector3.new(0, 0, 0) -- Hentikan momentum
                        end)
                    end
                end
            end
            task.wait() -- Kecepatan maksimal (RenderStepped)
        end
    end)

    -- 5. [EVENT START] PINDAH MAP
    shared.Reply.Connect("Meteor Shower Started", function(zoneName)
        if Config.AutoMeteor then
            ...
            if currentMap ~= zoneName then
                ...
                if Notify then Notify("Meteor", "Event started in " .. zoneName, "zap") end
                
                -- Teleport ke Map Event
                if Reply and Reply.To then Reply.To("Zone Teleport", zoneName) end
            end
        end
    end)

    -- 6. [EVENT PROCESS] SINKRONISASI WAKTU (HAPUS TELEPORT METEOR)
    shared.Reply.Connect("Meteor Spawn", function(impactPosition, zoneName)
        -- Di sini kita TIDAK teleport ke meteor.
        -- Kita hanya menunggu durasi jatuh agar script 'sadar' bahwa event sedang berlangsung.
        if Config.AutoMeteor then
            local fallDuration = shared.MeteorEvent and shared.MeteorEvent.METEOR_FALL_DURATION or 5
            task.wait(fallDuration) 
            -- Setelah wait ini selesai, biasanya server baru memunculkan/update musuh jadi 'Incandescent'.
            -- Saat itu terjadi, Loop Pemburu (No. 4) akan otomatis bekerja.
        end
    end)

    -- 7. [EVENT END] PULANG KAMPUNG
    shared.Reply.Connect("Meteor Shower Ended", function(zoneName)
        IncandescentTargets = {} -- Hapus semua target sisa
