...
function HookTimerVariable()
    task.spawn(function()
        -- Tunda 5 detik agar Script & UI selesai loading dulu baru scan memori
        task.wait(5) 
        
        local targetFound = false
        while not targetFound do
            -- Cek dulu apakah game sudah close/script mati
...
function LoadHeroesStatsData()
    local successStats, moduleStats = pcall(function()
        return require(RS.Scripts.Configs.MultipleZones.HeroesStats)
    end)
    ...
    local successHero, moduleHero = pcall(function()
        return require(RS.Scripts.Configs.MultipleZones.Heroes)
    end)
